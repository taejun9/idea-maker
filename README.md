...
`idea-maker`는 짧은 아이디어를 입력하면 구체화된 제품/사업 보고서로 정리해 주는 웹 서비스다. 사용자가 막연한 아이디어를 입력하면 시스템은 개념을 명확히 하고, 국내 경쟁 서비스와 해외 경쟁 서비스, 공개 스타트업/제품 디렉터리의 참고 사례를 분리해 보고서 형태로 제공한다.
...
2. 시스템이 아이디어를 구체적인 제품 개념으로 정리한다.
3. 국내 경쟁 서비스, 해외 경쟁 서비스, 인접 스타트업 사례를 수집하거나 fixture 기반 참고 자료로 보강한다.
4. 개요, 타깃 사용자, 핵심 사용 사례, 강점, 약점, 차별화 기회, 리스크, MVP 범위, 검증 체크리스트를 포함한 보고서를 생성한다.
...
루트 `README.md`는 항상 한국어로 프로젝트 전체를 요약한다. 프로젝트가 수정될 때마다 README의 내용, `버전`, `마지막 업데이트`가 현재 상태와 맞는지 확인하고 필요한 경우 같은 변경에서 갱신한다. `버전`은 별도 제품 버전 정책이 생기기 전까지 `pyproject.toml`의 프로젝트 버전을 따른다.
