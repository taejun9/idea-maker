...
- Quick examples are requested from the backend on page load. The API generates
  ...
  without external model calls, so refreshes stay fast and do not require model
  credentials.
...
- Local Gemma4 is scoped to organizing normalized research records for selected
  recommendations. Quick examples use a credential-free backend generator instead
  of Gemma, Gemini, or third-party source calls.
