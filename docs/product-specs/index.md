...
- Quick examples are requested from the backend on page load. The API selects
  random supported Q5 business fields except `기타`, asks local Gemma4 to write
  one concrete Korean idea example for each selected field, validates the JSON
  output, and falls back to deterministic examples when the model is unavailable
  or returns invalid output. The default response contains five examples.
...
- Local Gemma4 is scoped to organizing normalized research records for selected
  ...
  supported AI-context fields, and generating quick-example idea text from
  selected Q5 labels. Context and quick-example generation send only field
  labels, not raw user ideas, and deterministic fallback must complete the
  request when Gemma is unavailable.
