...
- Quick examples are sampled on page load from a curated pool aligned to the
  supported Q5 business fields except `기타`. They are local UI examples, not
  live Gemma output, so refreshes stay fast and do not require model credentials.
...
- Local Gemma4 is scoped to organizing normalized research records for selected
  recommendations. Generating quick examples with Gemma on every refresh requires
  a separate API, latency budget, fallback behavior, and user-data handling review
