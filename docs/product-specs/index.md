...
- Local Gemma4 is scoped to organizing normalized research records for selected
  ...
  supported AI-context fields, and generating quick-example idea text from the
  six allowed quick-example labels. Context and quick-example generation send
  only field labels, not raw user ideas, and deterministic fallback must complete
  the request when Gemma is unavailable.
