...
- Quick examples are requested from the backend on page load. The API generates
  five random examples from supported Q5 business-field contexts except `기타`.
  For IT, 교육, 금융, 라이프스타일, 마케팅/PR, and 미디어/엔터테인먼트, the
  backend attempts local Gemma4 business-context generation and falls back to
  deterministic context when the model is unavailable.
...
- For IT, 교육, 금융, 라이프스타일, 마케팅/PR, and 미디어/엔터테인먼트, the
  ...
  available before constructing those report sections.
...
- Local Gemma4 is scoped to organizing normalized research records for selected
  recommendations and generating business-field context from Q5 labels for the
  supported AI-context fields. Business-context generation sends only the field
  label, not the raw idea, and deterministic fallback must complete the request
